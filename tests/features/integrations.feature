...
Feature: Integrations scaffolding and error handling
  ...
  # ---------------------------------------------------------------------------
  @terraform @scaffold
  Scenario: Setup includes Terraform scaffold under gitops.git_dir/infra/terraform with documented tasks
    When I run "openCenter cluster setup --config-dir tmp/conf"
    Then the exit code should be 0
    And the directory "tmp/repo-dev/infra/terraform" should exist
    And the directory "tmp/repo-dev/infra/terraform" should contain a file matching "README.md"
    And the directory "tmp/repo-dev/infra/terraform" should contain a file matching "main\\.tf"
