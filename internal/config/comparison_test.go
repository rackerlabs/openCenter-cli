...
func TestCompareConfigs_SliceChanges(t *testing.T) {
	...
	// Modify slice length
	config2.OpenCenter.Cluster.K8sAPIPortACL = append(config2.OpenCenter.Cluster.K8sAPIPortACL, "10.0.0.0/8")
	...
	for _, change := range diff.Changes {
		if change.Path == "OpenCenter.Cluster.K8sAPIPortACL.length" && change.Type == ChangeTypeModified {
			foundLengthChange = true
		}
		if change.Path == "OpenCenter.Cluster.K8sAPIPortACL[1]" && change.Type == ChangeTypeAdded {
...
func TestCompareConfigs_EmptySliceVsNilSlice(t *testing.T) {
	...
	config1.OpenCenter.Cluster.K8sAPIPortACL = []string{}
	config2.OpenCenter.Cluster.K8sAPIPortACL = nil
