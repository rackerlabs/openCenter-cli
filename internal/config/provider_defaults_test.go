...
func TestNewProviderDefaultOpenStackHydratesRuntimeDefaults(t *testing.T) {
	...
	if cfg.OpenCenter.Infrastructure.Cloud.OpenStack.Region != "sjc3" {
		t.Fatalf("expected region sjc3, got %q", cfg.OpenCenter.Infrastructure.Cloud.OpenStack.Region)
	}
	...
	if cfg.OpenCenter.Cluster.Kubernetes.FlavorMaster != "gp.0.4.8" {
		t.Fatalf("expected hydrated master flavor, got %q", cfg.OpenCenter.Cluster.Kubernetes.FlavorMaster)
	}
	if cfg.OpenCenter.Cluster.Kubernetes.FlavorWorker != "gp.0.4.16" {
		t.Fatalf("expected hydrated worker flavor, got %q", cfg.OpenCenter.Cluster.Kubernetes.FlavorWorker)
	}
	if len(cfg.OpenCenter.Cluster.Networking.NTPServers) == 0 || cfg.OpenCenter.Cluster.Networking.NTPServers[0] != "time.sjc3.rackspace.com" {
