...
func TestCalicoPlugin_Metadata(t *testing.T) {
...
func TestCalicoPlugin_Status(t *testing.T) {
	plugin := NewCalicoPlugin()
...
func TestCalicoPlugin_Validate(t *testing.T) {
	plugin := NewCalicoPlugin()
...
func TestCalicoPlugin_BasePluginComposition(t *testing.T) {
	plugin := NewCalicoPlugin()

	// Verify metadata is accessible through the interface
	if plugin.Name() != "calico" {
		t.Errorf("Name() = %v, want calico", plugin.Name())
	}
