locals {
  ...
  openstack_tenant_name                   = "{{ .OpenCenter.Infrastructure.Cloud.OpenStack.TenantName | default "981977_Flex" }}"
  ...
  allocation_pool_end                     = "{{ .Networking.AllocationPoolEnd | default "" }}" != "" ? "{{ .Networking.AllocationPoolEnd }}" : cidrhost(local.subnet_nodes_cidr, -2)
  ...
  dns_nameservers                         = {{ if .Networking.DNSNameservers }}[{{ range $i, $dns := .Networking.DNSNameservers }}{{if $i}}, {{end}}"{{ $dns }}"{{ end }}]{{ else }}["8.8.8.8", "8.8.4.4"]{{ end }}
  ...
  image_id                                = "{{ .OpenCenter.Infrastructure.Cloud.OpenStack.ImageID | default "56277265-8f0c-40dc-87e2-944b7d320dae" }}"
  image_id_windows                        = "{{ .OpenCenter.Infrastructure.Cloud.OpenStack.ImageIDWindows | default "899af84f-d98f-4255-bf98-ceba5e3a8257" }}"
  ...
  flavor_worker_windows                   = "{{ .OpenCenter.Cluster.Kubernetes.FlavorWorkerWindows | default "gp.0.8.16" }}"

  worker_node_bfv_volume_size             = {{ .OpenCenter.Storage.WorkerVolumeSize | default 40 }}
  ...
  worker_node_bfv_volume_type             = "{{ .OpenCenter.Storage.WorkerVolumeType | default "HA-Standard" }}"
